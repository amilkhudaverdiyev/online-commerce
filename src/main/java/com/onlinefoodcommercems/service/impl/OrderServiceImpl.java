...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public List<Order> findByStatuss(OrderStatus status) {
        var order = orderRepository.findOrdersByStatus(status);
        log.error("order {}", order);
        return order;
    }
    ...
    @Override
    public void save(String username, LocalDateTime deliveryDate) throws MessagingException {
        ...
        if (ObjectUtils.isEmpty(carts)) {
            throw new NotDataFound(ResponseMessage.CART_NOT_FOUND);
        }
        ...
        for (OrderDetail orderDetail : orderEntity
            ...
            cartItemRepository.deleteCartItemByCustomer(customer.getId());
